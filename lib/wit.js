...
function Wit(opts) {
  ...
  const proxyAgent = getProxyAgent(witURL);
...
const getProxyAgent = (wit_url) => {
  ...
  const proxy = url.protocol === "http:" ? process.env.http_proxy || process.env.HTTP_PROXY : process.env.https_proxy || process.env.HTTPS_PROXY;
